import re
import base64
import zlib
import sys

charmap = []

def decode_base64_package(base64str):

	base64dec = base64.decodestring(base64str.encode())[20:]
	decoded = ''
	length = len(base64dec)

	for byte in range(0, length):
		extract = ord(base64dec[byte])
		charmap_extract = charmap[extract]
		decoded += chr(charmap_extract)

	return zlib.decompress(decoded)
	
sys.stderr.write("Running unwrapper \n\n" )

if len(sys.argv) < 2:
	sys.stderr.write("Usage: %s infile.sql [outfile]\n" % sys.argv[0])
	sys.exit(1)

infile = open(sys.argv[1])

if len(sys.argv) == 3:
	output_name = sys.argv[2]
else: 
	output_name = 'output.sql'

outfile = open(output_name, 'w')

lines = infile.readlines()
for i in range(0, len(lines)):
	matches = re.compile(r"^[0-9a-f]+ ([0-9a-f]+)$").match(lines[i])
	if matches:
		base64len = int(matches.groups()[0], 16)
		base64str = ''
		j = 0
		while len(base64str) < base64len:
			j+=1
			base64str += lines[i+j]
		base64str = base64str.replace("\n","")
		
		outfile.write( decode_base64_package(base64str))